...
export function CartOverlay() {
  ...
  const containerClass = isDesktop
    ? 'fixed right-0 top-0 bottom-0 z-50 flex w-[430px] max-w-[92vw] flex-col border-l border-white/10 bg-surface shadow-[-24px_0_60px_-10px_rgba(0,0,0,0.6)] animate-dc-drawer-in'
    : 'fixed inset-x-0 bottom-0 z-50 flex max-h-[88%] flex-col rounded-t-[30px] border-t border-white/[0.08] bg-surface shadow-[0_-20px_50px_-10px_rgba(0,0,0,0.6)] animate-dc-sheet-up';

  // The confirm button is pinned outside the scroll area so it stays reachable while a full
  // cart's line items scroll — the audit found it required scrolling all the way down otherwise.
  const showPinnedFooter = hasCart && !(isDesktop && deskConfirmed);
  const scrollAreaClass = [
    'dc-scroll flex-1 overflow-y-auto',
    isDesktop ? 'px-6 pt-7' : 'px-[22px] pt-3.5',
    showPinnedFooter ? 'pb-2' : 'pb-7',
  ].join(' ');
  ...
  return (
    <>
      ...
      <div className={containerClass}>
            ...
            <>
              ...
              <div className="mb-1.5 flex items-center justify-between">
                ...
                <button
                  ...
                  className="flex h-10 w-10 cursor-pointer items-center justify-center rounded-xs border-none bg-card-2 text-text transition-colors hover:bg-[#453769] lg:h-[38px] lg:w-[38px]"
                >
                  ...
                  <div className="mt-3">
                    {lines.map(({ product, qty }) => (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2 rounded-xs bg-bg p-1.5">
                          <button
                            onClick={() => decItem(product.id)}
                            aria-label="Diminuir quantidade"
                            className="flex h-9 w-9 cursor-pointer items-center justify-center rounded-[9px] border-none bg-card-2 text-text transition-transform active:scale-90"
                          >
                          ...
                          <button
                            ...
                            className="flex h-9 w-9 cursor-pointer items-center justify-center rounded-[9px] border-none bg-pink text-text transition-transform active:scale-90 disabled:opacity-40"
                          >
        ...
        {showPinnedFooter && (
          <div className={['shrink-0 border-t border-white/[0.06] pt-3', isDesktop ? 'px-6 pb-7' : 'px-[22px] pb-7'].join(' ')}>
            <button
              onClick={handleConfirm}
              disabled={submitting}
              className="relative flex h-[58px] w-full cursor-pointer items-center justify-center gap-2 overflow-hidden rounded-md border-none bg-gradient-to-br from-pink to-pink-dark font-display text-base font-semibold text-text shadow-[0_14px_30px_-10px_rgba(255,79,160,0.65)] transition-transform active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-60"
            >
